...
dataset_dir = '../search_engine_vidoseek/corpus'
port = 8002
...
async def startup_event():
    ...
    search_engine = SearchEngine(dataset_dir, node_dir_prefix='colqwen_ingestion', embed_model_name='/data/user0/models/vidore/colqwen2-v1.0')
...
async def search(queries: List[str] = Query(...)):
    ...
    results_batch = [[dict(idx=idx,image_file=(os.path.join(f'{dataset_dir}/img',file) if os.path.isfile(os.path.join(f'{dataset_dir}/img',file)) else file)) for idx,file in enumerate(query_results)] for query_results in results_batch]
